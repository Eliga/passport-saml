...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, isSign, callback) {
  ...
  Q.fcall(() => {
    ...
    .then(() => {
      ...
      if (isSign && this.options.privateCert) {
...
SAML.prototype.getAuthorizeUrl = function (req, options, callback) {
  this.generateAuthorizeRequest(req, this.options.passive, this.options.signXmlRequest, (err, request) => {
    if (err)
      return callback(err);
      ...
      if (encryptedAssertions.length == 1) {
        ...
        return Q.ninvoke(xmlenc, 'decrypt', encryptedAssertionXml, xmlencOptions)
          .then(decryptedXml => {
            ...
            const decryptionSignature = this.validateSignature(decryptedXml, decryptedAssertions[0], certs);

            if (this.options.cert && !validSignature && !decryptionSignature) {
              const regex = /(?<=<saml:EncryptedAssertion)(\s\S*)(?=>)/gm;
              const lastXML = xml.replace(encryptedAssertionXml.replace(regex, ""), decryptedXml);
              const lastDoc = new xmldom.DOMParser({}).parseFromString(lastXML);
              if (!this.validateSignature(lastXML, lastDoc.documentElement, certs)) {
